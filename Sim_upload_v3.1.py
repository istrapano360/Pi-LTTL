...
def cloud_check():										#V Cloud_online True/ False
	...
	global Cloud_online
	try:
		os.path.isdir("gdrive/Test") == True #TO DO: call variable directory from preset, in the preset will be defined the dir
	except:
		Cloud_online = False
		return Cloud_online
		print("Not mounted")#---------------------------
...
def upload_check():
	...
	if os.path.isdir("gdrive/Test/v1") == False:
		Error_upload = True
		return Error_upload
		print("Not uploaded ")#---------------------------
	else:# all good
...
def sync():
	...
	os.system("rclone --ignore-existing sync 'Local' gdrive -P -vv --dry-run")
	sleep(300)

def upload():
	print("rclone Upload function")#-------------------------------------
	os.system("rclone --checksum --ignore-existing sync 'Local' gdrive -P -vv --dry-run")
